...
public class Screen {
	...
	public void renderItems(int count) {
		...
		for (int i = 0; i < sprite.length; i++) {
			sprite[i] = Sprite.trans;
			if (Config.itemSpace[i] == 1) {
				sprite[i] = Sprite.key_blue;
			}
			if (Config.itemSpace[i] == 2) {
				sprite[i] = Sprite.key_red;
			}
			if (Config.itemSpace[i] == 3) {
				sprite[i] = Sprite.key_green;
			}
			if (Config.itemSpace[i] == 4) {
				sprite[i] = Sprite.potato;
			}
		}
			...
			for (int y = 0; y < 16; y++) {
				for (int x = 0; x < 16; x++) {
					...
					if (x < 0 || x >= width || y < 0 || y >= height)
						break;
					int col = sprite[i].pixels[x + y * 16];
					if (col != 0xFFB200FF && col != 0xFFFF00DC) {
						pixels[(xb) + (yb + (i * 16)) * width] = col;
					}
				}
			}
		}
	...
	public void renderPowers(int count) {
		Sprite[] sprite = new Sprite[4];
		for (int i = 0; i < sprite.length; i++) {
			sprite[i] = Sprite.trans;
		}
		if (Config.powerups[0] == 1) {
			sprite[0] = Sprite.pot;
		}
		if (Config.powerups[1] == 2) {
			sprite[1] = Sprite.crack;
		}
		if (Config.powerups[2] == 3) {
			sprite[2] = Sprite.pcp;
		}
		if (Config.powerups[3] == 4) {
			sprite[3] = Sprite.meth;
		}
		for (int i = 0; i < count; i++) {
			for (int y = 0; y < 16; y++) {
				for (int x = 0; x < 16; x++) {
	...
	public void drawDeath(Sprite sprite, int xa, int ya) {
		...
		drawText("Oh Dear, You are Dead!", (width / 2), (height / 2) - 7, 0xFFFFFFFF);
		for (int y = 0; y < 16; y++) {
			for (int x = 0; x < 16; x++) {
				int xb = xa + x;
				int yb = ya + y;
				...
				if (col != 0xFFD000FF && col != 0xFFFF00DC) {
					pixels[xb + (yb + (1 * 16)) * width] = col;
				}
			}
		}
		drawText("Press Enter to Continue", (width / 2) - 68, (height / 2) + 35, 0xFFFFFF00);
	}
